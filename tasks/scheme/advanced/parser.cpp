...
std::shared_ptr<Object> Read(Tokenizer* tokenizer) {
    ...
    } else if (next_token == Token{QuoteToken{}}) {
        tokenizer->Next();
        return_object = std::make_shared<Cell>(std::make_shared<Symbol>("quote"),
...
std::shared_ptr<Object> ReadList(Tokenizer* tokenizer) {
    ...
    std::shared_ptr<Cell> return_object(std::make_shared<Cell>(Read(tokenizer)));
    ...
    while (tokenizer->GetToken() != Token{BracketToken::CLOSE}) {
        if (tokenizer->GetToken() == Token{DotToken{}}) {
            ...
            current_object->SetSecond(std::make_shared<Cell>(Read(tokenizer)));
